...
!
! To convert from single to double:
! * Change wp
! * Change _double
         ...
         if(nrhs.gt.0) then
            ...
            if(m.gt.0) then
               ...
               call slv_fwd_update(m, n, dsa, offset, dnode%index,    &
                ...
                ! rhs_local(:,this_thread), ldr, rhs, ldr, buffer, control, id)
            endif
         endif
...
subroutine factor_diag_block(n, m, dest, zero_flag, control, dpotrf_info, &
      detlog, num_zero)
...
recursive subroutine tol_potrf(n, a, lda, zero_flag, diag_zero_minus, &
   ...
   if(n.le.nb) then
      ...
      do i = 1, n
         ...
         if ( i .lt. n ) then
            call dsyrk('U', 'T', (n-i), 1, -one, a(i*lda+i), lda, &
                 one, a(i*lda+i+1), lda)
         end if
      end do
   else
