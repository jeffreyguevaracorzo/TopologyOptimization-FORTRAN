...
! * Change wp
! * Change _single
! * Change BLAS calls: sgemv, sgemm, ssyrk, strsm, strsv
! * Change LAPACK calls: spotrf
!
...
!$ use omp_lib
   ...
   interface ma87_get_n__
      module procedure ma87_get_n_single
   end interface
...
subroutine MA87_factor_solve_one_single(n, ptr, row, val, order, keep, control,&
   ...
   call MA87_factor_solve_mult_single(n, ptr, row, val, order, keep, &
...
subroutine MA87_solve_one_single(x,order,keep,control,info,job)
   ...
   call MA87_solve_mult_single(1, keep%n, x, order, keep, &
      control, info, job)
...
subroutine MA87_solve_mult_single(nrhs,lx,x,order,keep, &
      control,info,job)
   integer, intent(in) :: nrhs ! number of right-hand sides to solver for.
   integer, intent(in) :: lx ! first dimension of x
   real(wp), intent(inout) :: x(lx,nrhs) ! On entry, x must
...
!
subroutine MA87_sparse_fwd_solve_single(nbi,bindex,b,order,invp,nxi,index,x,&
      w,keep,control,info)
...
!$OMP FLUSH(lfact)
    ...
    !     call factor_diag_block(n, m, id, lfact(bcol)%lcol(sa:sa+n*m-1),   &
         call factor_diag_block(n, m, lfact(bcol)%lcol(sa:sa+n*m-1),   &
         ...
         if(nrhs.gt.0) then
            ...
            if(m.gt.0) then
               ...
               call slv_fwd_update(m, n, dsa, offset, dnode%index,    &
                  ...
                  rhs_local(:,this_thread), ldr, rhs, ldr,            &
                  buffer)
             !     buffer, control, id)
            endif
         endif
...
!$OMP FLUSH
     ...
     !%%%    endif
      ...
      !        control)
         call solv_col_block(m, n, lfact(bcol)%lcol(sa:sa+n*m-1), &
              lfact(bcol)%lcol(sa1:sa1+n*m1), zero_flag(dsa:den), &
              control)
     ...
     !%%%    endif
      ...
      case(TASK_UPDATE_INTERNAL) ! update one block by two others
         ...
         call update_block_block(m, n, lfact(bcol)%lcol(sa:sa+n*m-1),  &
            ...
            lfact(bcol_src)%lcol(sa2:sa2+n1*m2-1))
...
!$OMP FLUSH(stack,rhs)
      ...
      case(TASK_SLV_FSLV) ! Perform forward solve with diagonal block
         ...
         call slv_solve(n, n, col, lfact(bcol)%lcol(sa:sa+n*n-1), &
            'Transpose    ', 'Non-unit', nrhs, rhs, ldr)
        ! 'Transpose    ', 'Non-unit', nrhs, rhs, ldr, control, blocks(blk)%id)
         offset = offset + n
      ...
      case(TASK_SLV_BSLV) ! Backward solve with diagonal block
         ...
         if(m.gt.n) then
            call slv_bwd_update(m-n, n, col, offset+n, nodes(node)%index, &
               ...
               rhs_local(:,this_thread), ldr, xlocal)
         ...
         ! Perform triangular solve
         call slv_solve(n, n, col, lfact(bcol)%lcol(sa:sa+n*n-1), &
            'Non-Transpose', 'Non-unit', nrhs, rhs,               &
             ldr)
         !    ldr, control, blocks(blk)%id)
...
!
! TASK_SOLVE_BLOCK
! Solve using factorization of diag. block (uses strsm)
! A_ij <- A_ij A_ii^-1
! dest <- dest diag^-1
!
!subroutine solv_col_block(m, n, id, dest, id1, diag, zero_flag, control)
subroutine solv_col_block(m, n, dest, diag, zero_flag, control)
 ...
 !%%%  integer :: t_start, t_end, this_thread
  ...
  !%%% if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   call strsm('Left', 'Upper', 'Transpose', 'Non-Unit', n, m, &
      one, diag, n, dest, n)
...
!
! TASK_FACTORIZE_BLOCK (uses Lapack routine spotrf and strsm)
! A_ii <- L_ii
!
!subroutine factor_diag_block(n, m, id, dest, zero_flag, control, spotrf_info, &
subroutine factor_diag_block(n, m, dest, zero_flag, control, spotrf_info, &
      detlog, num_zero)
 ...
 !  integer(long), intent(in) :: id ! block identifier for dest
   ...
   type(MA87_control), intent(in) :: control
   integer, intent(out) :: spotrf_info ! error flag for spotrf
 ...
 !%%%  if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   if(control%diag_zero_minus.ge.control%diag_zero_plus .and. &
      ...
      ! Call LAPACK routine
      call spotrf('Upper', n, dest, n, spotrf_info)
   else
      ! Call custom routine (slower)
      call tol_potrf(n, dest, n, zero_flag, control%diag_zero_minus, &
         control%diag_zero_plus, spotrf_info, num_zero)
   endif
   ! check for errors
   if(spotrf_info.ne.0) return
   ...
   ! Do strsm with any remainder below diagonal block
   if(m.gt.n) then
...
recursive subroutine tol_potrf(n, a, lda, zero_flag, diag_zero_minus, &
   ...
   if(n.le.nb) then
      ...
      do i = 1, n
         ...
         ! Fix: update is not performed after the last pivot.
         if (i .lt. n) then
            call ssyrk('U', 'T', (n-i), 1, -one, a(i*lda+i), lda, one, &
                 a(i*lda+i+1), lda)
         end if
      end do
   else
      ...
      ! Apply to A_21
      call strsm('L', 'U', 'T', 'N', i, n-i, 1.0_wp, a, lda, a(i*lda+1), lda)
      do j = 1, i
         if(zero_flag(j)) a(i*lda+j : n*lda : lda) = 0.0
      end do

      ! Update A_22
      call ssyrk('U', 'T', n-i, i, -1.0_wp, a(i*lda+1), lda, 1.0_wp, &
         a(i*lda+i+1), lda)
...
subroutine update_block_block(m, n, dest, blk, n1, src1, src2)
   integer, intent(in) :: m ! number of rows in dest
   integer, intent(in) :: n ! number of columns in dest
...
!$ call omp_set_lock(blk%alock)
   ...
   if(diag) then
      call ssyrk('U', 'T', n, n1, -one, src1, n1, one, dest, n)

      if(m.gt.n) then
         ! Do a sgemm on any remainder
         call sgemm('T', 'N', n, m-n, n1, -one,                   &
            src1, n1, src2(1+n*n1), n1, one, dest(1+n*n), n)
      endif
   else
      ! dest is an off-diagonal block
      call sgemm('T', 'N', n, m, n1, -one, src1, n1, src2, n1, one, dest, n)
   endif
 ...
 10   if (st.ne.0) then
   ...
   if(n1.ge.control%min_width_blas) then
      ...
      if(diag) then
         ...
         call ssyrk('U', 'T', ndiag, n1, -one, csrc,                  &
            n1, zero, buffer, col_list_sz)

         if(s2en-s2sa+1-ndiag.gt.0) then
            call sgemm('T', 'N', ndiag, int(s2en-s2sa+1-ndiag),       &
         ...
         ! Off diagonal block
         ndiag = 0
         call sgemm('T', 'N', int(s1en-s1sa+1), int(s2en-s2sa+1), n1, &
...
!%%%   if(control%unit_log.gt.0) call system_clock(t_start)
   ...
   if(nrhs.eq.1) then
      call strsv('Upper', trans, unit, nelim, dest, n, rhs(col), 1)
   else
      call strsm('Left', 'Upper', trans, unit, nelim, nrhs, one, &
         dest, n, rhs(col), ldr)
   endif
...
pure integer function ma87_get_n_single(keep)
   type(ma87_keep), intent(in) :: keep
   ma87_get_n_single = keep%n
end function ma87_get_n_single
